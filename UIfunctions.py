...
def showcard(card_ui, card):
    ...
    image = QPixmap.fromImage(card.getgraphic())
...
def showback(card_ui):
    ...
    image = QImage(path + '/cards/back.png')
    pixmap = QPixmap(image)
    card_ui.setPixmap(pixmap)
    card_ui.setHidden(False)
    return
